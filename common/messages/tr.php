<?php 
	$messages = array(
			"accessDenied" 		=> "Yetkiniz Yok.",
			"notFound" 			=> "Bulunamadı.",
			"redirecting"     	=> "Yönlendiriliyorsunuz lütfen bekleyin.",
			"successSignUp" 	=> "Başarı ile kayıt oldunuz.",
			"errorLogin" 		=> "Giriş bilgileriniz hatalı.",
			"successLogin" 		=> "Başarı ile giriş yapıldı.",
			"sessionTimeOut"    => "Oturum süreniz sona ermiş yeniden giriş yapınız.",

			//validation messages
			"notEmpty" 			=> "%name% boş bırakılamaz.",
			"invalid" 			=> "Geçerli bir %name% giriniz.",
			"stringMax" 		=> "%name% %max% karakterden fazla olmamalıdır.",
			"stringMin" 		=> "%name% %min% karakterden az olmamalıdır.",
			"alreadyUsed" 		=> "Girilen %name% daha önce kullanılmış.",
			"check" 			=> "Lütfen kontrol ediniz.",
			"notSamePassword" 	=> "Girilen şifreler uyuşmamaktadır.",
	);
?>